...
::: {#def-cov-prob}
The *covariance* of a pair of discrete random variables $X$ and $Y$ with the joint probability distribution $P(x,y)$ is:
$$ Cov(X,Y) = E((X-\mu_X)(Y-\mu_Y)) = \sum_i \sum_j (x_i-\mu_x)(y_j-\mu_y)P(x_i,y_j)$$
where the double sum is added up over all possible pairs of values of the random variables $X$ and $Y$: $(x_i, y_j)$.
:::
...
$$ Var(X) = E(X^2)  - E(X)^2 = 42/12 - 9/4 = (42-27)/16 = 15/16$$ a
nd the standard deviation is $\sigma = \sqrt{15}/4$ or just under 1.
...
The formula for the binomial distribution in @eq-binom-rv provides the answer for any given number of mutants. For example, the probability of 3 people out of 20 being carriers for the mutation is: 
