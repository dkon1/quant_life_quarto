...
## Vectors and plotting in R {#sec-comp2}
...
### Calculations with vectors and plotting

In this tutorial you will learn to:

* Assign vector variables and perform calculations using vectors
* Make plots using vectors or function expressions

## Vector variables and indexing

Variables may contain more than a single number, they can also store a bunch of numbers, which is then called an *array*. When numbers in an array are organized as a single ordered list, this is called a *vector*.
...
### c() function 
There are several ways of producing a vector of numbers in R. The first is to put together several numbers by listing them inside the function `c()`:
```{webr-r}
vec<-c(pi,45,912.8, 0)
print(vec)
print(vec[2])
...
Sometimes you want to create a vector of repeated values. For example, to create a variable with 20 zeros you can use `rep()` like this:
...
This command extracted the fifth and second element of the vector `vec1`, in that order. One can also use the colon to extract a range of vector elements:
...
print(vec2[4:7])
```
The script above extracts the fourth through the seventh element of `vec2` in order and prints them.
...
The following R commands or short scripts contain errors; your job is to fix them so they do what each exercise asks you to do.
...
Hint: print function requires (); indexing vectors requires [].
...
Hint: use `:` to create a vector of ten indices.
...
Hint: use `seq()` to create a vector of odd numbered indices.
...
Hint: multiplication must be specified by `*`.
...
Hint: check the order of inputs in seq
...
Hint: the independent variable must be the first input and the dependent variable must be the second input
...
Hint: the second graph should be made with lines() and color red; in `legend` change the 'pch' and 'lty' options.
