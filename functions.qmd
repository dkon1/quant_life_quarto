...
1.  Number of mutations $M$ as a function of time $t$: $$ M(t) = M_0 + \mu t$$

2.  Baby weight $B$ as a function of time $t$:

    $$
    B(t) = rt+a
    $$

3.  Molecular concentration $C$ as a function of time $t$: $$ C(t) = C_0 e^{-kt} $$

4.  Tree height $H$ (length) as a function of age $a$ (time): $$ H(a) = \frac{b a}{c + a}$$

5.  Concentration of a gene product $G$ as a function of time $t$: $$ G(t) = G_m (1 - e^{-\alpha t})$$

6.  Reaction rate $c$ (concentration/time) as a function of concentrations of reactant $A$ and reactant $B$:

    $$
    c(A,B) = kAB
    $$

7.  Enzyme turnover rate $v$ (1/time) as a function of concentration of substrate $S$: $$ v(S) = \frac{v_{max} S}{K_m + S}$$

8.  Population $P$ as a function of time $t$: $$ P(t) = \frac{Ae^{rt}}{B + e^{rt}} $$

9.  Fraction of bound receptors $\theta$ as a function of concentration of ligand $L$: $$ \theta(L) = \frac{L^n}{L^n + K_d}$$

10. Mass of an organism $M$ as a function of the basal metabolic rate (energy/time) $B$:
