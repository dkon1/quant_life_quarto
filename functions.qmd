...
7.  Logistic model of population growth, $P$ is population size and time $t$: $$ P(t) = \frac{A}{1 + Be^{-rt}} $$
...
The numerator and denominator both contain exponential functions with the same power. If $r>0$ when $x \to \infty$, the denominator approaches 1, since the exponential term decays to zero, and thus \$l: $$  
\lim_{x \to \infty} \frac{A}{1+Be^{-rx}} = A; \; \mathrm{if} \; r>0 
$$

On the other hand, if $r<0$, the exponential term in the denominator grows to infinity and the ratio approaches zero: $$  \lim_{x \to \infty} \frac{A}{1+Be^{-rx}} =  0; \; \mathrm{if} \; r<0 $$
