# Tutorial 2: Data frames and descriptive statistics {.unnumbered}
...
-   Visualize variables using box plots
...
A **data frame** is an object in R environment that contains multiple variables with different names. These variables can be accessed by using the name of the data frame, followed by `$` and then the variable name, e.g. `df$var1` refers to variable named `var1` in data frame `df`. Each variable usually contains multiple values and is thus a *vector*, and may be of different *types* (for example numbers, characters, Booleans).
:::
