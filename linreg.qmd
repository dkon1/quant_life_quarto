...
: Body leanness (B) and heat loss rate (H) in boys; partial data set from @sloan_cooling_1973 {#tab-ch4-bh}
...
@fig-penguins-hr shows scatterplots of the rate of oxygen consumption (VO) and heart rate (HR) measured in two macaroni penguins running on a treadmill (really). The authors performed linear regression on the data and found the following parameters: $VO =0.23HR - 11.62$ (penguin A) and $VO =0.25HR - 20.93$ (penguin B). The datasets have the standard deviations: $\sigma_{VO} = 6.77$ and $\sigma_{HR} = 28.8$ (penguin A) and $\sigma_{VO} = 8.49$ and $\sigma_{HR} = 30.6$ (penguin B).

![Mass-specific rate of oxygen consumption (VO) as a function of heart rate (HR) in two macaroni penguins, (A) a breeding female of mass 3.14 kg and a moulting female of mass 3.99 kg; figure from under CC-BY.](ch8/penguin_HR.png){#fig-penguins-hr}
...
plot(my_data$Temp, my_data$HR, xlab = "body temperature (deg F)" , ylab = "heart rates (bpm)", main= 'scatterplot and linear regression line')
abline(myfit)
# plot the residuals
plot(myfit$fitted.values, myfit$residuals, xlab = "predicted temp (deg F)" , ylab = "residuals (deg F)", main='residuals plot')
abline(0,0)
```
