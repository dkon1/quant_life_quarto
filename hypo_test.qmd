...
## Simulation of multiple tests for independence between genotype and disease
...
## -----------------------------------------------------------------
## Shiny app: simulating many chi-squared tests of independence
## between genotype (A / B) and phenotype (D = disease / N = normal).
##
## Each simulation:
##   - draws `samp_size` phenotypes for genotype A with P(disease) = probA
##   - draws `samp_size` phenotypes for genotype B with P(disease) = probB
##   - cross-tabulates into a 2x2 table and runs chisq.test()
## Across `n_sims` repetitions we count how many tests are
## "positive" (p < alpha, reject independence) vs "negative".
##
## When probA == probB the null (independence) is TRUE, so the
## positive rate estimates the Type I error rate (~ alpha).
## When probA != probB the positive rate estimates the test's POWER.
## -----------------------------------------------------------------

library(shiny)

dis_states <- c("D", "N")   # 'D' = disease, 'N' = normal

## Run n_sims chi-squared tests; return the vector of p-values ------
run_sims <- function(n_sims, samp_size, probA, probB) {
  vapply(seq_len(n_sims), function(i) {
    gA <- sample(dis_states, samp_size, replace = TRUE, prob = c(probA, 1 - probA))
    gB <- sample(dis_states, samp_size, replace = TRUE, prob = c(probB, 1 - probB))

    # Force both levels so the table is always 2x2, even if a
    # category happens not to appear in a given sample.
    tA <- table(factor(gA, levels = dis_states))
    tB <- table(factor(gB, levels = dis_states))
    m  <- cbind(A = tA, B = tB)   # rows = D / N, cols = genotype A / B

    # Extreme probabilities can produce an all-zero row (undefined test);
    # capture that as NA rather than letting it error out the run.
    tryCatch(
      suppressWarnings(chisq.test(m)$p.value),
...
ui <- fluidPage(
  ...
  titlePanel("Simulating many chi-squared tests: genotype vs. phenotype"),

  sidebarLayout(

    sidebarPanel(
      sliderInput("probA", "P(disease) for genotype A",
                  min = 0, max = 1, value = 0.1, step = 0.01),
      sliderInput("probB", "P(disease) for genotype B",
                  min = 0, max = 1, value = 0.4, step = 0.01),
      sliderInput("samp_size", "Sample size per genotype",
                  min = 10, max = 1000, value = 200, step = 10),
      sliderInput("alpha", "Significance level (alpha)",
                  min = 0.001, max = 0.1, value = 0.05, step = 0.001),
      sliderInput("n_sims", "Number of simulated tests",
    ...
    mainPanel(
      fluidRow(
        column(4, wellPanel(
          h4("Positive"),
          helpText("p < alpha (reject independence)"),
          h2(textOutput("n_pos")))),
        column(4, wellPanel(
          h4("Negative"),
          helpText("p >= alpha"),
        ...
        column(4, wellPanel(
          h4("Positive rate"),
          helpText(textOutput("rate_label")),
          h2(textOutput("pos_rate"))))
      ),
      plotOutput("pval_hist", height = "340px"),
...
server <- function(input, output, session) {
  ...
  # Re-run the whole batch when any input changes or the button is clicked.
  pvals <- reactive({
    input$resample
    run_sims(input$n_sims, input$samp_size, input$probA, input$probB)
  })
  ...
  counts <- reactive({
    ...
    valid <- p[!is.na(p)]
    list(
      pos     = sum(valid <  input$alpha),
      neg     = sum(valid >= input$alpha),
      n_valid = length(valid),
      n_bad   = sum(is.na(p))
    )
  })

  output$n_pos <- renderText(counts()$pos)
  output$n_neg <- renderText(counts()$neg)

  output$pos_rate <- renderText({
    cc <- counts()
    if (cc$n_valid == 0) return("NA")
    sprintf("%.1f%%", 100 * cc$pos / cc$n_valid)
  })

  output$rate_label <- renderText({
    if (isTRUE(all.equal(input$probA, input$probB)))
      "Type I error (null true)"
    else
      "Power (real effect)"
  })
  ...
  output$pval_hist <- renderPlot({
    ...
    hist(p, breaks = seq(0, 1, 0.05),
         col = "grey80", border = "white", xlim = c(0, 1),
         xlab = "p-value",
         main = "Distribution of p-values across simulations")
    abline(v = input$alpha, col = "firebrick", lwd = 2, lty = 2)
    legend("topright",
           legend = sprintf("alpha = %.2f", input$alpha),
           col = "firebrick", lwd = 2, lty = 2, bty = "n")
  })

  output$interpretation <- renderUI({
    cc   <- counts()
    same <- isTRUE(all.equal(input$probA, input$probB))
    msg  <- if (same) {
      paste0("Genotypes A and B have the SAME disease probability, so genotype ",
             "and phenotype are truly independent. Positive results here are ",
             "false positives, and the positive rate should hover near alpha.")
    } else {
      paste0("Genotypes A and B have DIFFERENT disease probabilities, so a real ",
             "association exists. The positive rate estimates the test's power ",
             "to detect it at this sample size and alpha.")
    }
    bad <- if (cc$n_bad > 0)
      paste0(" (", cc$n_bad, " test(s) returned an undefined p-value from an ",
             "all-zero row and were excluded.)")
    else ""
    helpText(paste0(msg, bad))
  })
}
...
1.  Set two very different probabilities (e.g. 0.3 and 0.1). At significance level of 0.01, what sample size is sufficient for 90% true positive rate?
2.  Set two very different probabilities (e.g. 0.3 and 0.1). At significance level of 0.001, what sample size is sufficient for 90% true positive rate?
3.  Set two closer probabilities (e.g. 0.3 and 0.25). At significance level of 0.01, what sample size is sufficient for 90% true positive rate?
4.  Set two closer probabilities (e.g. 0.3 and 0.25). At significance level of 0.001, what sample size is sufficient for 90% true positive rate?
5.  When the two probabilities are equal, what does the p-value distribution look like? What is the false positive rate you observe at different significance levels?
