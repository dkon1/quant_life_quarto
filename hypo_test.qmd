...
| Health/Genotype | Genotype A | Genotype B |
|-----------------|------------|------------|
| Disease         | 40         | 20         |
| Healthy         | 80         | 60         |

: Simulated data containing variables of genotype (A or B) and health (Disease or Healthy)

@tbl-gen-dis shows a simulated contingency table for two variables: genotype and disease.

1.  Calculate the marginal probabilities (proportions) of the individual random variables, i.e. the probability of disease and healthy in the data set, and of Genotype A and B.

2.  Calculate the probability of Disease given genotype A and the probability of Disease given genotype B.

3.  Calculate the probability of genotype B given Healthy and the probability of genotype B given Disease.

4.  Assuming the disease and genotype are independent, calculate the expected number of genotype A and Disease.

5.  Under the same assumption, calculate the expected number of genotype B and Disease.

6.  Under the same assumption, calculate the expected number of genotype A and Healthy.

7.  Under the same assumption, calculate the expected number of genotype B and Healthy.
...
## T-test and hypothesis testing errors

The t-test is a way of testing whether two samples have the same means. The null hypothesis is that $\mu_1 = \mu_2$, and the test calculates a number called the t-statistic, which is a difference of the two sample means divided by the a combination of standard deviations of the two samples:

$$t = \frac{\bar X_1 - \bar X_2}{\sqrt{s_1^2/N_1 + s_2^2/N_2}}$$

The test uses the t-distribution to compute the p-value for the null hypothesis.

For example, consider samples of size $n$ taken from two normal distributions (with unobserved means $\mu_1$, $\mu_2$). We can generate the data using a function:
...
In this code chunk we control the sample size of both samples (assuming it's the same) and the two true means of the samples, and perform the Student's t-test to decide whether the means differ significantly:

```{webr-r}
samp_size <- 100 
mean1 <- 1
mean2 <- 2
my_sample <- generate_samples(samp_size, mean1, mean2)
...
1.  For sample size 100, what is the typical p-value for samples with the true mean values of 1 and 2?
2.  For sample size 100, what is the typical p-value for samples with the true mean values of 1 and 1.5?
3.  For sample size 100, what is the typical p-value for samples with the true mean values of 1 and 1.1?
4.  For sample size 100, what is the typical p-value for samples with the true mean values of 1 and 1.01?
5.  For sample size 100, what is the typical p-value for samples with identical true mean values?

### Comparing heart rates

Let us load the Heart Rates data set we first saw on Day 1, which contains the variables: `Rest1`, `Ex1`, `Rest2`, `Ex2`, and `Year`. Then we print out the number of observations (rows) in the data frame - each row is a different student who responded to the survey.

```{webr-r}
heart_rates <- read.csv(file = "https://raw.githubusercontent.com/dkon1/quant_life_quarto/master/data/HR_data_combined.csv")
head(heart_rates)
cat("Number of observations in heart_rates:", nrow(heart_rates))
```

This code chunk extracts the values of the heart rates for years 2025 and 2026 and performs the t-test between the values of Ex1 in the two years:

```{webr-r}
heart_rates2025 <- subset(heart_rates, Year == 2025)
heart_rates2026 <- subset(heart_rates, Year == 2025)

t.test(heart_rates2025$Ex1, 
       heart_rates2026$Ex1)
```

### Questions

1.  Perform the t-test on years 2026 and 2024 and interpret the result.
2.  Perform the t-test on years 2026 and 2023 and interpret the result.
3.  Perform the t-test on years 2026 and 2022 and interpret the result.
4.  Perform the t-test on years 2026 and 2022 and interpret the result.
5.  Perform the t-test on years 2026 and 2021 and interpret the result.
6.  Perform the t-test on years 2026 and 2020 and interpret the result.

### T-tests for samples from different distributions
...
## ---------------------------------------------------------------
## Two-Sample Comparison Simulator
## Draws two independent samples from a chosen distribution family
## (each group with its own parameters), displays side-by-side
## boxplots, and reports the p-value from a two-sample t-test
...
ui <- fluidPage(
  ...
  titlePanel("Two-Sample Comparison: Boxplot & t-test"),
      ...
      fluidRow(
        column(
          ...
          conditionalPanel(
            ...
            numericInput("a_norm_mean", "Mean", value = 0),
            numericInput("a_norm_sd",   "Std. dev.", value = 1, min = 0.0001)
          ),
          conditionalPanel(
            condition = "input.distribution == 'uniform'",
            numericInput("a_unif_min", "Minimum", value = 0),
            numericInput("a_unif_max", "Maximum", value = 1)
          ),
          conditionalPanel(
            condition = "input.distribution == 'binomial'",
            numericInput("a_binom_size", "Trials (n)", value = 20, min = 1, step = 1),
            sliderInput("a_binom_prob", "Prob. of success (p)",
                        min = 0, max = 1, value = 0.5, step = 0.01)
          ),
          conditionalPanel(
            condition = "input.distribution == 'exponential'",
            numericInput("a_exp_rate", "Rate", value = 1, min = 0.0001)
          )
        ),
        ...
        column(
          ...
          conditionalPanel(
            condition = "input.distribution == 'normal'",
            numericInput("b_norm_mean", "Mean", value = 0.5),
            numericInput("b_norm_sd",   "Std. dev.", value = 1, min = 0.0001)
          ),
          conditionalPanel(
            condition = "input.distribution == 'uniform'",
            numericInput("b_unif_min", "Minimum", value = 0.3),
            numericInput("b_unif_max", "Maximum", value = 1.3)
          ),
          conditionalPanel(
            condition = "input.distribution == 'binomial'",
            numericInput("b_binom_size", "Trials (n)", value = 20, min = 1, step = 1),
            sliderInput("b_binom_prob", "Prob. of success (p)",
                        min = 0, max = 1, value = 0.65, step = 0.01)
          ),
          conditionalPanel(
            condition = "input.distribution == 'exponential'",
            numericInput("b_exp_rate", "Rate", value = 1.5, min = 0.0001)
          )
        )
      ),

      hr(),
      actionButton("resample", "Draw new samples", icon = icon("dice"))
    ),
...
server <- function(input, output, session) {
  ...
  # Draw the two samples. Regenerated when the distribution, sample
  # size, any relevant parameter changes, or the button is clicked.
  samples <- reactive({

    input$resample  # dependency: re-draw on button click

    n <- input$n

    if (input$distribution == "normal") {
      a <- rnorm(n, mean = input$a_norm_mean, sd = input$a_norm_sd)
      b <- rnorm(n, mean = input$b_norm_mean, sd = input$b_norm_sd)

    } else if (input$distribution == "uniform") {
      a <- runif(n, min = input$a_unif_min, max = input$a_unif_max)
      b <- runif(n, min = input$b_unif_min, max = input$b_unif_max)

    } else if (input$distribution == "binomial") {
      a <- rbinom(n, size = input$a_binom_size, prob = input$a_binom_prob)
      b <- rbinom(n, size = input$b_binom_size, prob = input$b_binom_prob)

    } else if (input$distribution == "exponential") {
      a <- rexp(n, rate = input$a_exp_rate)
      b <- rexp(n, rate = input$b_exp_rate)
    }

    list(a = a, b = b)
  })

  # Run the two-sample (Welch) t-test on the current samples
  ...
  output$boxPlot <- renderPlot({
    ...
    boxplot(
      ...
      main   = paste0(
        dist_label, " samples, n = ", input$n, " each\n",
        "t-test p-value = ", signif(pval, 4)
      ),
      ylab = "Value"
    )

    # Overlay group means as points
    points(
      x   = c(1, 2),
      y   = c(mean(s$a), mean(s$b)),
      pch = 18, cex = 1.8, col = "firebrick"
    )
    legend("topright", legend = "Group mean", pch = 18, col = "firebrick", bty = "n")
  })
