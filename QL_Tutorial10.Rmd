---
title: "Quantifying Life R Tutorial 10"
output: 
  learnr::tutorial:
    css: css/style.css
...
trans2to1 <- 0.8 # transition probability from 2 to 1
decider <- runif(1) # random number between 0 and 1
if (state==1) {
	if (decider < trans1to2) { # randomly decide to transition
		state <- 2
...
```{r ex2-1-hint}
# matrices are assigned in columns by default
```


```{r ex2-1-solution}
A <- matrix (c(0,-5,1,10),nrow=2)
print(A) 
```
...
```{r ex2-3-hint}
# place only the matrix multiplication line inside the loop
```
...
barplot(state_count,main='frequency of states vs. time',xlab='time', names.arg=1:(numsteps+1),ylab='state counts',col=1:numstates)
```
