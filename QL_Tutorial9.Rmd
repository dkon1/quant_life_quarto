---
title: "Quantifying Life R Tutorial 9"
output: 
  learnr::tutorial:
    css: css/style.css
...
We will use the package `deSolve` to calculate numeric solutions of ODEs using a function that you define yourself. First, install the package `deSolve` (you can use the Install button in the Packages tab in R Studio) and then load it at the beginning of your R script or Rmd document with the following command:

```{r}
library(deSolve)
```

Second, we need to create the R function that defines the derivative in the differential equation, in other words, the function $f(x,t)$ in the generic first-order ODE
